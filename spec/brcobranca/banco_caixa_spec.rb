...
describe Brcobranca::Boleto::Caixa do #:nodoc:[all]
  ...
  it 'Criar nova instância com atributos padrões' do
    ...
    carteira = "#{described_class::MODALIDADE_COBRANCA[:sem_registro]}" <<
        "#{described_class::EMISSAO_BOLETO[:beneficiario]}"
  ...
  describe '#carteira_boleto' do
    ...
    it "Retorna 'RG' quando é uma carteira registrada" do
      boleto_novo = described_class.new valid_attributes.merge(:carteira => '14')
  ...
  describe '#valor_documento' do
    ...
    context "quando" do
      context "informado" do
        before { valid_attributes.merge! valor_documento: '22222.22' }

        it { subject.valor_documento.should eql 22222.22 }
      end

      context "NÃO informado" do
        it { subject.valor_documento.should eql 10.00 }
      end
    end
  end
end
